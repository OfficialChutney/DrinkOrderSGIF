...
public sealed class DrinkService(AppDbContext dbContext) : IDrinkService
{
    ...
    public async Task<Drink> AddDrinkAsync(string name, int price, int unitsPerPrice, int? klipsPrice, CancellationToken cancellationToken = default)
    {
    ...
    public async Task UpdateDrinkAsync(Guid drinkId, string name, int price, int unitsPerPrice, int? klipsPrice, CancellationToken cancellationToken = default)
    {
        var trimmed = name.Trim();
        if (string.IsNullOrWhiteSpace(trimmed))
        {
